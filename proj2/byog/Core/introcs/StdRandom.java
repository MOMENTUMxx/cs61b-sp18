...
public final class StdRandom {
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 2) edu.princeton.cs.introcs.StdRandom.setSeed(Long.parseLong(args[1]));
        double[] probabilities = { 0.5, 0.3, 0.1, 0.1 };
        int[] frequencies = { 5, 3, 1, 1 };
        String[] a = "A B C D E F G".split(" ");

        StdOut.println("seed = " + edu.princeton.cs.introcs.StdRandom.getSeed());
        for (int i = 0; i < n; i++) {
            ...
            edu.princeton.cs.introcs.StdRandom.shuffle(a);
            for (String s : a)
                StdOut.print(s);
            StdOut.println();
        }
