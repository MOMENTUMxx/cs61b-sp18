...
public final class Draw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
   ...
   /**
    ...
    public static void main(String[] args) {
        ...
        edu.princeton.cs.introcs.Draw draw1 = new edu.princeton.cs.introcs.Draw("Test client 1");
        draw1.square(.2, .8, .1);
        draw1.filledSquare(.8, .8, .2);
        draw1.circle(.8, .2, .2);
        draw1.setPenColor(edu.princeton.cs.introcs.Draw.MAGENTA);
        draw1.setPenRadius(.02);
        draw1.arc(.8, .2, .1, 200, 45);


        // create another one
        edu.princeton.cs.introcs.Draw draw2 = new edu.princeton.cs.introcs.Draw("Test client 2");
        draw2.setCanvasSize(900, 200);
        // draw a blue diamond
        draw2.setPenRadius();
        draw2.setPenColor(edu.princeton.cs.introcs.Draw.BLUE);
        double[] x = { .1, .2, .3, .2 };
        double[] y = { .2, .3, .2, .1 };
        draw2.filledPolygon(x, y);

        // text
        draw2.setPenColor(edu.princeton.cs.introcs.Draw.BLACK);
        draw2.text(0.2, 0.5, "bdfdfdfdlack text");
        draw2.setPenColor(edu.princeton.cs.introcs.Draw.WHITE);
